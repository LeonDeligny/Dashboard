"""
Module related to the different graphs in Micro-Machining > Shift.
...
def register_shift_callbacks(app):
    ...
    ################################################
    # 1st Graph : Machining: NOK by Type per Shift #
    ################################################

    @app.callback(
        [
        Output('Machining: NOK by Type per Shift', 'figure'),
        Output('Machining: NOK by Type per Shift (bis)', 'figure'),
        ],
        [
    ...
    def update_1(selected_year, selected_week, operator_value, equipment_value, shift_value, weekday_value, equipment_per_operator):
        ...
        title = generate_title(input_values, "Machining: NOK by Type per Shift")
        ...
        fig.add_trace(go.Scatter(hovertemplate='%{y:.2f}', mode="markers+lines",  x=team_ratio_df['Shift'], y=team_ratio_df['NOK (%)'], name='NOK (%)', yaxis='y2', line=dict(color=COLOR_DICT['NOK (%)'])))
    ...
    ####################################################
    # 2nd Graph : Machining: NOK (%) Boxplot per Shift #
    ####################################################

    @app.callback(
        [
        Output('Machining: NOK (%) Boxplot per Shift', 'figure'),
        Output('Machining: NOK (%) Boxplot per Shift (bis)', 'figure'),
        ],
        [
    ...
    def update_2(selected_year, selected_week, operator_value, equipment_value, equipment_per_operator, shift_value, weekday_value):
        ...
        title = generate_title(input_values, "Machining: NOK (%) Boxplot per Shift")
...
def shift_layout(operator_show):
    ...
    if operator_show:
        ...
        dbc.Row([               
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Machining: NOK by Type per Shift', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Machining: NOK by Type per Shift (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
...
"""
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Machining: NOK (%) Boxplot per Shift', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Machining: NOK (%) Boxplot per Shift (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
