"""
Module related to the different graphs in Micro-Machining > Equipment.
...
import dash_bootstrap_components as dbc, plotly.graph_objects as go, numpy as np, pandas as pd
...
from dicts import CustomCard, WEEK_DICT, EQUIPMENTS_PER_OPERATOR_LIST, EQUIPMENTS_PER_OPERATOR_LABEL_LIST, SHIFT_LABEL_LIST, NOK_SORTING_LIST, WEEKDAY_LABEL_LIST, COLOR_DICT
from utils import load_operator_label, load_equipment_label, extract_type_smc

def register_equipment_callbacks(app):

    ####################################################
    # 1st Graph : Machining: NOK by Type per Equipment #
    ####################################################

    @app.callback(
        [
        Output('Machining: NOK by Type per Equipment', 'figure'),
        Output('Machining: NOK by Type per Equipment (bis)', 'figure'),
        ],
        [
    ...
    def update_1(selected_year, selected_week, operator_value, equipment_value, shift_value, weekday_value, equipment_per_operator):
        ...
        title = generate_title(input_values, "Machining: NOK by Type per Equipment")
    ...
    ################################################
    # 2nd Graph : Machining: NOK (%) per Equipment #
    ################################################

    @app.callback(
        [
        Output('Machining: NOK (%) per Equipment', 'figure'),
        Output('Machining: NOK (%) per Equipment (bis)', 'figure'),
        ],
        [
    ...
    def update_2(selected_year, selected_week, operator_value, equipment_value, equipment_per_operator, shift_value, weekday_value):
        ...
        title = generate_title(input_values, "Machining: NOK (%) per Equipment")
    ...
    ########################################################
    # 3rd Graph : Machining: NOK (%) Boxplot per Equipment #
    ########################################################

    @app.callback(
        [
        Output('Machining: NOK (%) Boxplot per Equipment', 'figure'),
        Output('Machining: NOK (%) Boxplot per Equipment (bis)', 'figure'),
        ],
        [
    ...
    def update_3(selected_year, selected_week, operator_value, equipment_value, shift_value, equipment_per_operator, weekday_value):
        ...
        df_smc = load_df_smc(input_values)
        filtered_df = filter_dataframe(df_smc, input_values)
        title = generate_title(input_values, "Machining: NOK (%) Boxplot per Equipment")
...
def equipment_layout(operator_show):
    ...
    if operator_show:
        ...
        dbc.Row([               
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Machining: NOK by Type per Equipment', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Machining: NOK by Type per Equipment (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
...
"""
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Machining: NOK (%) per Equipment', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Machining: NOK (%) per Equipment (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Machining: NOK (%) Boxplot per Equipment', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Machining: NOK (%) Boxplot per Equipment (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
