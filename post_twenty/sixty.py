"""
Module related to the different graphs in Post Processing > 60.
...
from pp_load import load_df_sixty
...
def register_sixty_callbacks(app):

    #############################################
    # 1st Graph : 60: NOK by Type per week #
    #############################################

    @app.callback(
        [
        Output('60: NOK by Type per week', 'figure'),
        Output('60: NOK by Type per week (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Week_Bar(selected_year, selected_week, selected_operator, selected_equipment):
        ...
        title = generate_title(input_values, "60: NOK by Type per week")

        df_sixty = load_df_sixty(input_values)
        df_sixty_type = extract_type(df_sixty)

        filtered_df = filter_dataframe(df_sixty, input_values)
        filtered_df_type = filter_dataframe(df_sixty_type, input_values)
    ...
    #################################################
    # 2nd Graph : 60: NOK by Type per Operator #
    #################################################

    @app.callback(
        [
        Output('60: NOK by Type per Operator', 'figure'),
        Output('60: NOK by Type per Operator (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Operator_Bar(selected_year, selected_week, selected_operator, selected_equipment):
        input_values = {'Year': selected_year, 'Week': selected_week, 'Operator': selected_operator, 'Equipment': selected_equipment}
        title = generate_title(input_values, "60: NOK by Type per Operator")
    ...
    #####################################################
    # 3rd Graph : 60: NOK by Type per Collaborator #
    #####################################################

    @app.callback(
        [
        Output('60: NOK by Type per Collaborator', 'figure'),
        Output('60: NOK by Type per Collaborator (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Collaborator_Bar(selected_year, selected_week, selected_equipment):
        ...
        title = generate_title(input_values, "60: NOK by Type per Collaborator")
    ...
    ##################################################
    # 4th Graph : 60: NOK by Type per Equipment #
    ##################################################

    @app.callback(
        [
        Output('60: NOK by Type per Equipment', 'figure'),
        Output('60: NOK by Type per Equipment (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Equipment_Bar(selected_year, selected_week, selected_equipment, selected_operator):
        ...
        title = generate_title(input_values, "60: NOK by Type per Equipment")
    ...
    #########################################
    # 5th Graph : 60: NOK per Type #
    #########################################

    @app.callback(
        [
        Output('60: NOK per Type', 'figure'),
        Output('60: NOK per Type (bis)', 'figure')
        ],
        [
    ...
    def update_5(selected_year, selected_week, selected_equipment):
        input_values = {'Year': selected_year, 'Week': selected_week, 'Equipment': selected_equipment}
        title = generate_title(input_values, "60: NOK per Type")
        
        df_fifty = load_df_sixty(input_values)
    ...
    #########################################################
    # 6rd Graph : % of ofas that went to 60 #
    #########################################################

    @app.callback(
        [
        Output('% of ofas that went to 60', 'figure'),
        Output('% of ofas that went to 60 (bis)', 'figure'),
        ],
        [
    ...
    def update_6(selected_year, selected_week, equipment_value):
        ...
        annotations = [dict(x=0.1, y=-0.1, xref='paper', yref='paper', text=f'Total Parts controlled at 60 Operation for weeks {selected_week_start} to {selected_week_end}: {total_pcs}.', showarrow=False)]

        fig = go.Figure(data=[go.Pie(labels=['60', 'Not Sorted'], values=[size_final_df_bis, size_neg_final_df_bis], hole=.3)])
    ...
    def update_7(selected_year, selected_week, equipment_value):
        ...
        df = df_erp_load()
        mapping = df[df['Operation'] == 'Ten'].set_index('ofa')['Equipment'].to_dict()
        ...
        final_df_group = final_df.groupby(["Equipment"]).size().reset_index(name='Counts')
...
def sixty_layout(operator_show):
    ...
    if operator_show:
        ...
        dbc.Row([               
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='60: NOK by Type per week', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='60: NOK by Type per week (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='60: NOK by Type per Operator', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='60: NOK by Type per Operator (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='60: NOK by Type per Collaborator', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='60: NOK by Type per Collaborator (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='60: NOK by Type per Equipment', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='60: NOK by Type per Equipment (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='60: NOK per Type', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='60: NOK per Type (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        html.Br(),
        html.P("Data comes from database.db2."),

        dbc.Row([

            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='% of ofas that went to 60', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='% of ofas that went to 60 (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
