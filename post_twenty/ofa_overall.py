...
Database name : PCdatabase.db2 (Oracle) & hsma2.
"""
...
from dicts import CustomCard, COLOR_DICT_EQUIPMENT, WEEK_DICT, OPERATIONS_LIST
from utils import load_equipment_label, extract_type

def register_ofa_overall_callbacks(app):

    ###################################################################
    # 1st Graph : NOK (%) from each Ten Equipment per Operation #
    ###################################################################

    @app.callback(
        [
        Output('NOK (%) from each Ten Equipment per Op', 'figure'),
        Output('NOK (%) from each Ten Equipment per Op (bis)', 'figure'),
        ],
        [
    ...
    def update_1(selected_year, selected_week, equipment_value):
        ...
        df_erp = df_erp_load()
        title = generate_title(input_values, "NOK (%) from each Ten Equipment per Op")
    ...
    ###################################################################
    # 2nd Graph : NOK (%) from each Ten Equipment per Operation #
    ###################################################################
    ...
    def update_2(selected_year, selected_week, equipment_value):
        ...
        fig.add_trace(go.Scatter(hovertemplate='%{y:.2f}', mode="markers+lines",  x=team_ratio_df['Operation'], y=team_ratio_df['NOK (%)'], name='NOK (%)', yaxis='y2', line=dict(color='black')))
        if NOK_Ratio.size > 0:
...
def ofa_overall_layout():
    ...
    html.Br(),
    html.P("Left graph's data comes from database.db2, and right graph's data comes from database1.db."),
     
    dbc.Row([               

        ############################################################
        # 1st Graph : NOK (%) from each Ten Equipment per Op #
        ############################################################

        dbc.Col([
            CustomCard([
                dbc.Col(dcc.Graph(id='NOK (%) from each Ten Equipment per Op', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
            ...
            dbc.Modal([
                ...
                dbc.ModalBody(
                    CustomCard([
                        dcc.Graph(id='NOK (%) from each Ten Equipment per Op (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
