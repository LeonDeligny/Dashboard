"""
Module related to the different graphs in Post Processing > Release 100%.
...
from pp_load import load_df_release
...
def register_release_callbacks(app):

    #############################################
    # 1st Graph : Release 100%: NOK by Type per week #
    #############################################

    @app.callback(
        [
        Output('Release 100%: NOK by Type per week', 'figure'),
        Output('Release 100%: NOK by Type per week (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Week_Bar(selected_year, selected_week, selected_operator, selected_equipment):
        ...
        title = generate_title(input_values, "Release 100%: NOK by Type per week")

        df_release = load_df_release(input_values)
        df_release_type = extract_type(df_release)

        filtered_df = filter_dataframe(df_release, input_values)
        filtered_df_type = filter_dataframe(df_release_type, input_values)
    ...
    #################################################
    # 2nd Graph : Release 100%: NOK by Type per Operator #
    #################################################

    @app.callback(
        [
        Output('Release 100%: NOK by Type per Operator', 'figure'),
        Output('Release 100%: NOK by Type per Operator (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Operator_Bar(selected_year, selected_week, selected_operator, selected_equipment):
        input_values = {'Year': selected_year, 'Week': selected_week, 'Operator': selected_operator, 'Equipment': selected_equipment}
        title = generate_title(input_values, "Release 100%: NOK by Type per Operator")
    ...
    #####################################################
    # 3rd Graph : Release 100%: NOK by Type per Collaborator #
    #####################################################

    @app.callback(
        [
        Output('Release 100%: NOK by Type per Collaborator', 'figure'),
        Output('Release 100%: NOK by Type per Collaborator (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Collaborator_Bar(selected_year, selected_week, selected_equipment):
        ...
        title = generate_title(input_values, "Release 100%: NOK by Type per Collaborator")
    ...
    ##################################################
    # 4th Graph : Release 100%: NOK by Type per Equipment #
    ##################################################

    @app.callback(
        [
        Output('Release 100%: NOK by Type per Equipment', 'figure'),
        Output('Release 100%: NOK by Type per Equipment (bis)', 'figure')
        ],
        [
    ...
    def Update_PP_Trier_Type_Equipment_Bar(selected_year, selected_week, selected_equipment, selected_operator):
        ...
        title = generate_title(input_values, "Release 100%: NOK by Type per Equipment")
    ...
    #########################################
    # 5th Graph : Release 100%: NOK per Type #
    #########################################

    @app.callback(
        [
        Output('Release 100%: NOK per Type', 'figure'),
        Output('Release 100%: NOK per Type (bis)', 'figure')
        ],
        [
    ...
    def update_5(selected_year, selected_week, selected_equipment):
        input_values = {'Year': selected_year, 'Week': selected_week, 'Equipment': selected_equipment}
        title = generate_title(input_values, "Release 100%: NOK per Type")
        
        df_dimensional = load_df_release(input_values)
    ...
    #########################################################
    # 6rd Graph : % of OFAs that went to Release 100% #
    #########################################################

    @app.callback(
        [
        Output('% of OFAs that went to Release 100%', 'figure'),
        Output('% of OFAs that went to Release 100% (bis)', 'figure'),
        ],
        [
    ...
    def update_6(selected_year, selected_week, equipment_value):
        ...
        df_erp = df_erp_load()
        title = generate_title(input_values, "% of OFAs that went to Release 100%")
        ...
        df_erp = df_erp[(df_erp['Operation Step'] == 71) | (df_erp['Operation Step'] == 72)]
        final_df = filter_dataframe(df_erp, input_values)
        ofa_to_keep = final_df.loc[((final_df['Operation Step'] == 71) & (final_df['NOK'] != 0)) | ((final_df['Operation Step'] == 72) & (final_df['NOK'] != 0) ), 'OFA'].unique()
        final_df_bis = final_df.loc[final_df['OFA'].isin(ofa_to_keep)]
        neg_final_df_bis = final_df.loc[~final_df['OFA'].isin(ofa_to_keep)]

        final_df_bis = final_df_bis[final_df_bis['Operation Step'] == 71]
        ...
        annotations = [dict(x=0.1, y=-0.1, xref='paper', yref='paper', text=f'Total Parts controlled at Release 100% Operation for weeks {selected_week_start} to {selected_week_end}: {total_pcs}.', showarrow=False)]

        fig = go.Figure(data=[go.Pie(labels=['Release 100%', 'Not'], values=[size_final_df_bis, size_neg_final_df_bis], hole=.3)])
        fig.update_layout(height=400, title_text='<b>' + title + '</b>', annotations=annotations)
        return fig, fig

    #########################################################
    # 7th Graph : % of Equipments that got to Release 100% #
    #########################################################

    @app.callback(
        [
        Output('% of Equipments that got to Release 100%', 'figure'),
        Output('% of Equipments that got to Release 100% (bis)', 'figure'),
        ],
        [
    ...
    def update_7(selected_year, selected_week, equipment_value):
        input_values = {'Year': selected_year, 'Week': selected_week,'Equipment': equipment_value}
        title = generate_title(input_values, "% of Equipments that got to Release 100%")
        df = df_erp_load()
        ...
        df = df[((df['Operation Step'] == 71) | (df['Operation Step'] == 72)) & (df['NOK'] != 0)]
...
def release_layout(operator_show):
    ...
    if operator_show:
        ...
        dbc.Row([               
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Release 100%: NOK by Type per week', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Release 100%: NOK by Type per week (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Release 100%: NOK by Type per Operator', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Release 100%: NOK by Type per Operator (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Release 100%: NOK by Type per Collaborator', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Release 100%: NOK by Type per Collaborator (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Release 100%: NOK by Type per Equipment', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Release 100%: NOK by Type per Equipment (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='Release 100%: NOK per Type', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='Release 100%: NOK per Type (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
        ...
        dbc.Row([
            ...
            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='% of OFAs that went to Release 100%', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='% of OFAs that went to Release 100% (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"flexGrow": "1"}),
            ...
            #########################################################
            # 4th Graph : % of Equipments that got to Release 100% #
            #########################################################

            dbc.Col([
                CustomCard([
                    dbc.Col(dcc.Graph(id='% of Equipments that got to Release 100%', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}), className='p-0', style={"flexGrow": "1"}),
                ...
                dbc.Modal([
                    ...
                    dbc.ModalBody(
                        CustomCard([
                            dcc.Graph(id='% of Equipments that got to Release 100% (bis)', clickData=None, config={'scrollZoom':True, 'displayModeBar': False}, style={"margin": "auto", "display": "block"}),
